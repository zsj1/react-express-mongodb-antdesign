import axios from "axios";
import { getRedirectPath } from '../util'
const AUTH_SUCCESS = "AUTH_SUCCESS";
const LOGOUT = 'LOGOUT'
const ERROR_MSG = "ERROR_MSG";
const LOAD_DATA = "LOAD_DATA";
const REDIRECT = "REDIRECT";
const REGISTER_SUCCESS = "REGISTER_SUCCESS";
const RECORD = "RECORD"
const initState = {
  redirectTo: '',
  msg: "",
  user: "",
  type: "",
  lastPage: "",
};
// reducer
export function user(state = initState, action) {
  switch (action.type) {
    case AUTH_SUCCESS:
      return { ...state, msg: "", redirectTo:getRedirectPath(action.payload), ...action.payload };
    case REGISTER_SUCCESS:
      return { ...initState, redirectTo:getRedirectPath(action.payload), ...action.payload }
    case ERROR_MSG:
      return { ...state, isAuth: false, msg: action.msg };
    case LOAD_DATA:
      return {...state, ...action.payload}
    case LOGOUT:
      return {...initState, redirectTo:'/login'}
    case REDIRECT:
      return { ...state, redirectTo: `/${state.type}info` }
    case RECORD:
    return { ...state, lastPage: action.payload.lastPage }
    default:
      return state;
  }
}
function authSuccess(obj) {
  const {pwd,...data} = obj;
  return {type: AUTH_SUCCESS, payload:data}
}
function registerSuccess(obj) {
  const {pwd, ...data} = obj;
  return {type: REGISTER_SUCCESS, payload: data}
}
function errorMsg(msg) {
  return { msg, type: ERROR_MSG };
}
export function loadData({userinfo}){
  return {type:LOAD_DATA, payload:userinfo};
}
export function logoutSubmit(){
	return { type:LOGOUT }
}
export function updateRedirect(){
  return { type:REDIRECT }
}
export function record(lastPageUrl) {
  return { type:RECORD, payload:{lastPage: lastPageUrl} }
}
export function register({ user, pwd, repeatpwd, type }) {
  if (!user || !pwd || !type) {
    return errorMsg("用户名密码必须输入");
  }
  if (pwd !== repeatpwd) {
    return errorMsg("密码和确认密码不同");
  }
  return dispatch => {
    axios
      .post("/user/register", {
        user: user,
        pwd: pwd,
        type: type
      })
      .then(res => {
        if (res.status === 200 && res.data.code === 0) {
          dispatch(registerSuccess({ user, pwd, type }));
        } else {
          dispatch(errorMsg(res.data.msg));
        }
      });
  };
}

export function login({ user, pwd }) {
    if (!user || !pwd) {
        return errorMsg("用户名密码必须输入");
    }
    return dispatch => {
        axios
          .post("/user/login", {user, pwd})
          .then(res => {
            if (res.status === 200 && res.data.code === 0) {
              dispatch(authSuccess(res.data.data));
            } else {
              dispatch(errorMsg(res.data.msg));
            }
          });
      };
}

export function update(data){
    return dispatch => {
        axios.post('/user/update',data)
        .then(res => {
            if (res.status === 200 && res.data.code === 0) {
              // console.log(res.data.data)
              dispatch(authSuccess(res.data.data));
            } else {
              dispatch(errorMsg(res.data.msg));
            }
          });
    };
}

export function getInfo() {
  return dispatch => {
    axios.get('/user/info')
    .then(res => {
      if (res.status === 200 && res.data.code === 0) {
        // console.log(res.data.data);
        dispatch(authSuccess(res.data.data));
      } else {
        dispatch(errorMsg(res.data.msg));
      }
    });
  }
}